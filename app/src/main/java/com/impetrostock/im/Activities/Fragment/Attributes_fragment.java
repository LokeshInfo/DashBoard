...
public class Attributes_fragment extends Fragment
{
    ...
    Attributes_fragment attributes_fragment;
    ...
    {
        Add_Attributes adAttr = new Add_Attributes();
        getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.fragment_layout,adAttr).addToBackStack("Add Attr").commit();
    }

    private void prdata()
    {
        for (int t=1 ; t<11 ; t++) {
            dataList.add(new SubCategory_Data("CAT_Name-"+t, "Sub Cat_Name"+t, "Cat_Fields Colors..."));
        }
